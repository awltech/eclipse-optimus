...
public class ParentContextElementInjector extends Injector {
	...
	public ParentContextElementInjector(Field field) {
		this.field = field;
		ParentContextElement annotation = field.getAnnotation(ParentContextElement.class);
	...
	public void inject(AbstractTransformation<?> transformation, ITransformationContext context)
		...
		if (this.isInjectable() && parent != null) {
			...
			OptimusM2MEngineMessages.CI07.log(transformation.getClass().getName(), this.field.getName(),
					eObjectLabelProvider.getText(parent));
		} else {
	...
	public void update(AbstractTransformation<?> transformation, ITransformationContext context)
		...
		if (this.isUpdatable() && parent != null) {
			...
			if (object instanceof EObject) {
				context.put(parent, this.mapping, (EObject) object);
				OptimusM2MEngineMessages.CI09.log(transformation.getClass().getName(), this.field.getName(),
						eObjectLabelProvider.getText((EObject) object));
			} else {
