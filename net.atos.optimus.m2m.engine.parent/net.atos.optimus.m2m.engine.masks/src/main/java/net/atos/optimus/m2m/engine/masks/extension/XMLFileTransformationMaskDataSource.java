...
import net.atos.optimus.m2m.engine.masks.UserTransformationMaskCreationTool;
	...
	protected void loadTransformationUserMasks() {
		...
		if (transformationMaskDirectory.exists()) {
			...
			for (String transformationName : this.transformationMaskReferences.keySet()) {
				File transformationMaskFile = UserTransformationMaskCreationTool.giveAssociatedXMLFile(transformationName);
			...
			for (File transformationMaskFile : transformationMaskDirectory.listFiles()) {
				String transformationMaskName = UserTransformationMaskCreationTool.giveAssociatedMaskName(transformationMaskFile);
